...
import org.huha.bsresponsewrap.module.exception.BusinessExceptionEnum;
...
public class DefaultErrorResult implements Result {
    ...
    public static DefaultErrorResult failure(BusinessException e) {
        ...
        if (ee != null) {
            return DefaultErrorResult.failure(e.getResultCode(), e, ee.getHttpStatus(), e.getData());
        }

        DefaultErrorResult defaultErrorResult = DefaultErrorResult.failure(e.getResultCode() == null ? ResultCode.FAILURE : e.getResultCode(), e, HttpStatus.OK, e.getData());
